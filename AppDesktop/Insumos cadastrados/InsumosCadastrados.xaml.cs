...
namespace Agrotech.Insumos_cadastrados;

public partial class InsumosCadastrados : ContentPage
{
	public InsumosCadastrados()
	{
		InitializeComponent();
	}

    private async void Button_Numero_Lote_insumo(object sender, EventArgs e) {
        ...
        try {
            ...
            DataTable loteInsumo = await buscaBanco.BuscarLotesPorNumeroLoteInsumoAsync(NumeroLoteEntry.Text);

            // Verifica se há dados retornados
            if (loteInsumo.Rows.Count > 0) {
                List<LoteProduto> listaLotes = new List<LoteProduto>();

                foreach (DataRow row in loteInsumo.Rows) {
    ...
    private async void Button_Datainicial_final_insumo(object sender, EventArgs e) {
        ...
        try {
            ...
            DataTable loteProduto = await buscaBanco.BuscarLotesPorDataInsumoIntervaloAsync(dataInicial, dataFinal);
    ...
    private async void Button_Insumo(object sender, EventArgs e) {
        // Verifica se o nome do produto foi informado
        if (string.IsNullOrWhiteSpace(PesquisaInsumo.Text)) {
        ...
        try {
            ...
            DataTable loteProduto = await buscaBanco.BuscarLotesPorNomeInsumoAsync(PesquisaInsumo.Text);
    ...
    private async void Button_Datadevencimento_insumo(object sender, EventArgs e) {
        ...
        try {
            ...
            DataTable loteProduto = await buscaBanco.BuscarLotesPorDataVencimentoInsumoAsync(dataVencimento);
